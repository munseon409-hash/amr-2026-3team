...
def serialcomming(data_in):
    global odom_pub, cnt2, prev_x, prev_y, prev_yaw, prev_time, init, odom, t
    checksum = 0x00
    for x in range(22):
      checksum += data_in[x] 
    ...
    #-------------------------------------------------------------------------------------------*/
      ...
      if outlier == False:
        ...
        # set the velocity (computed from position delta)
        odom.child_frame_id = "base_footprint"
        dt = (current_time - old_time).nanoseconds / 1e9
        if dt > 0.001:
          dx = New_Px - old_x
          dy = New_Py - old_y
          vx = (dx * math.cos(yaw) + dy * math.sin(yaw)) / dt
          wz = (yaw - old_yaw) / dt
          odom.twist.twist.linear.x  = max(-2.0, min(2.0, vx))
          odom.twist.twist.angular.z = max(-5.0, min(5.0, wz))
        else:
...
def main(args=None):
  ...
  global prev_yaw, prev_time, init, odom, t
